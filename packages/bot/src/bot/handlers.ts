...
async function getLatestSessionProfile(
  deps: HandlerDeps,
  chatId: number
): Promise<
  | { error: Reply }
  | { sessionId: string; profile: ProfileRecord }
> {
...
export async function handleCodexEffortMenu(
  deps: HandlerDeps,
  chatId: number,
  telegramUser: number
): Promise<Reply> {
  ...
  if (current.profile.tool !== "OPENAI") {
    return {
      text:
        "🧠 Effort selection is only available for Codex profiles.\n\n" +
...
export async function handleSetCodexEffort(
  deps: HandlerDeps,
  chatId: number,
  telegramUser: number,
...
export async function handleWorkDirPicked(
  deps: HandlerDeps,
  chatId: number,
  telegramUser: number,
  result: { index: number } | { skip: true }
): Promise<Reply> {
  const state = deps.flows.get(chatId, telegramUser);
...
export async function handleFolderMenu(
  deps: HandlerDeps,
  chatId: number,
  telegramUser: number
): Promise<Reply> {
  ...
  if (workDirs.length === 0) {
    return {
      text:
        "This daemon has no working directories configured.\n\n" +
...
export async function handleFolderPicked(
  deps: HandlerDeps,
  chatId: number,
  _telegramUser: number,
  result: { index: number } | { useDefault: true }
): Promise<Reply> {
  const session = await deps.sessions.getLatestActiveByChatId(chatId);
  if (!session) {
    return {
