...
import { clearState, isProcessAlive, killProcess, killRegisteredTools, readState, writeState } from "./daemonState.js";
 ...
 */
async function sweepStaleInstances(log) {
...
async function runDaemon() {
    ...
    process.on("exit", () => {
        ...
        if (!state || state.daemonPid !== process.pid)
            return;
    ...
    const sweepTimer = setInterval(() => {
        void (async () => {
            ...
            if (state.daemonPid !== process.pid)
                return;
