 ...
 */
let codexRootOverride = null;
let sourceCodexHomeOverride = null;
export function setCodexRootOverride(root) {
    codexRootOverride = root;
}
export function setSourceCodexHomeOverride(root) {
    sourceCodexHomeOverride = root;
}
export function codexHomeRoot() {
    if (codexRootOverride !== null)
        return codexRootOverride;
    return path.join(os.homedir(), ".chatcoder-daemon", "codex");
}
function sourceCodexHome() {
    if (sourceCodexHomeOverride !== null)
        return sourceCodexHomeOverride;
    return path.join(os.homedir(), ".codex");
}
export function codexHomeFor(profileName) {
    return path.join(codexHomeRoot(), profileName);
}
/** Template for the per-profile config.toml. */
export function renderCodexConfigToml(codex) {
...
export function renderCodexAuthJson(codex) {
    if (!codex.apiKey) {
...
function resolvedConfigContent(codexHome, codex) {
    if (codex.baseUrl)
...
function resolvedAuthContent(codexHome, codex) {
    if (codex.apiKey)
        return renderCodexAuthJson(codex);
 ...
 */
export function ensureCodexHome(profileName, codex) {
