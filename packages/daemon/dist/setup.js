...
export const validators = {
    ...
    profileName: (v) => /^[A-Za-z0-9][A-Za-z0-9_.-]*$/.test(v)
        ? true
...
function messagePlacementChoiceIndex(placement) {
    if (placement === "stdin")
        return 1;
...
function outputFormatChoiceIndex(format) {
    return format === "stream-json" ? 1 : 0;
}
function seedProfiles(existing) {
    const out = [];
...
function syncOpenAiProfileHomes(cfg) {
...
function showSetupProfiles(ui, page, selectedSlot, profiles) {
    ...
    for (let idx = start; idx < end; idx += 1) {
        ...
        out(`${ui.CARD_ACCENT}${prefix} [${slot + 1}]${ui.RESET} ${ui.BOLD}${p.name}${ui.RESET}` +
...
async function pickTool(ui, initialTool) {
    const options = [
    ...
    const selected = await pickWithArrows(ui, "Select Tool", "Use arrow keys to choose a tool, then press Enter.", options, toolChoiceIndex(initialTool));
...
async function promptClaude(ui, prev) {
    ...
    if (skipPermissions === null)
        return null;
    const outputFormat = await pickWithArrows(ui, "Output Format", "Select output format.", [
...
async function promptOpenAi(ui, prev) {
    const editAuth = await askYesNo(ui, "Edit authentication for this profile?", prev?.apiKey === undefined);
    if (editAuth === null)
        return null;
    let apiKey = prev?.apiKey;
    let baseUrl = prev?.baseUrl;
    if (editAuth) {
        const nextApiKey = await askValidated(ui, "OPENAI_API_KEY: ", prev?.apiKey ?? "", validators.nonEmpty);
        if (nextApiKey === null)
            return null;
        apiKey = nextApiKey;
        const nextBaseUrl = await askWithDefault(ui, "OPENAI_BASE_URL (blank = OpenAI default): ", prev?.baseUrl ?? "");
        if (nextBaseUrl === null)
            return null;
        if (nextBaseUrl.length > 0) {
            const ok = validators.apiUrl(nextBaseUrl);
            if (ok !== true) {
                printWarning(ui, ok);
                return promptOpenAi(ui, { ...prev, apiKey, baseUrl: nextBaseUrl });
            }
        }
        baseUrl = nextBaseUrl || undefined;
    }
    ...
    if (model === null)
        return null;
    const fullAuto = await askYesNo(ui, "Use --full-auto (sandbox=workspace-write, approval=never)?", prev?.fullAuto ?? true);
    if (fullAuto === null)
        return null;
    const bypassApprovalsAndSandbox = await askYesNo(ui, "Use --dangerously-bypass-approvals-and-sandbox?", prev?.bypassApprovalsAndSandbox ?? false);
    if (bypassApprovalsAndSandbox === null)
        return null;
    return {
        apiKey,
        baseUrl,
...
async function promptCustom(ui, prev) {
    const launchBin = await askValidated(ui, "Launch binary (path or name): ", prev?.launchBin ?? "", (value) => {
        const nonEmpty = validators.nonEmpty(value);
        if (nonEmpty !== true)
            return nonEmpty;
        if (/\s/.test(value))
            return "Use a single binary name or path without spaces";
        return true;
    });
    if (launchBin === null)
        return null;
    const argsRaw = await askWithDefault(ui, "Args (space-separated; blank for none): ", prev?.args.join(" ") ?? "");
    if (argsRaw === null)
        return null;
    const messagePlacement = await pickWithArrows(ui, "Message Placement", "Where should the instruction text go?", [
    ...
    if (messagePlacement === null || messagePlacement === "__BACK__")
        return null;
    const envRaw = await askWithDefault(ui, "Extra env (KEY=value space-separated; blank for none): ", prev ? formatEnvRaw(prev.env) : "");
    if (envRaw === null)
        return null;
    return {
        launchBin,
...
async function promptProfileEditor(ui, existing, takenNames) {
    ...
    if (tool === null)
        return null;
    const name = await askValidated(ui, "Profile name: ", existing?.name ?? "", (value) => {
...
async function addProfile(ui, profiles) {
...
async function updateProfile(ui, profiles, index) {
...
async function deleteProfile(ui, profiles, index) {
    const existing = profiles[index];
    if (!existing) {
        printWarning(ui, "No profile selected");
        await pause(ui);
...
async function manageProfiles(ui, profiles) {
    let page = 0;
    let selectedSlot = 0;
    for (;;) {
        const total = profiles.length;
        if (total === 0) {
            ...
            if (lastSlot >= LANDING_PAGE_SIZE)
                lastSlot = LANDING_PAGE_SIZE - 1;
...
async function runSetupCoderStyle(existing, targetPath) {
    ...
    const profiles = seedProfiles(existing);
...
function printBannerSimple(io) {
    io.log(LINE);
    io.log("  chatcoder coder --setup");
    io.log(LINE);
}
function printSectionSimple(io, title) {
    io.log("");
    io.log(title);
    io.log(LINE);
}
function printProfilesSimple(io, profiles) {
...
async function promptOneProfilePromptWizard(io, index, takenNames, existing) {
    ...
    return {
        ...
        custom: {
            ...
            args: parseArgsRaw(c.argsRaw),
            env: parseEnvRaw(c.envRaw),
            messagePlacement: c.messagePlacement ?? "appended"
        }
    };
}
async function runSetupPromptWizard(existing, io = defaultIO, targetPath = defaultConfigPath()) {
    clearIfTty();
    printBannerSimple(io);
...
export async function runSetup(existing, io = defaultIO, targetPath = defaultConfigPath()) {
