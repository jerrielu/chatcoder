...
import {
  ...
  promptApiUrl,
  promptApiKey,
  promptMaxConcurrency
} from "./setup.js";
import type { CoderUi } from "./setup.js";
...
function defaultConfig(): Partial<DaemonConfig> {
  return {
...
function loadOrCreateCfg(): Partial<DaemonConfig> {
  ...
  return {
    ...
    heartbeatIntervalMs:
      typeof raw.heartbeatIntervalMs === "number" ? raw.heartbeatIntervalMs : 15000,
    idleShutdownMs:
      typeof raw.idleShutdownMs === "number" ? raw.idleShutdownMs : 3600000,
    workDirs:
      Array.isArray(raw.workDirs) ? (raw.workDirs as string[]) : []
  };
}

function saveCfg(cfg: Partial<DaemonConfig>): void {
  writeRawConfig(cfg as Record<string, unknown>);
}
...
function showProfiles(
  ...
  out(
    ui,
    `${ui.BOLD}Select a profile to activate${ui.RESET}`
  );
  out(
    ui,
    `${ui.DIM}Use arrow keys to select, Enter to activate, shortcuts for actions.${ui.RESET}`
  );
  out(ui, "");
...
function showWorkDirs(ui: CoderUi, workDirs: string[]): void {
...
function showFooter(ui: CoderUi): void {
  out(ui, "");
  printLine(ui);
...
function out(ui: CoderUi, lineText = ""): void {
  process.stdout.write(lineText + "\n");
}

function printProfileSummary(ui: CoderUi, p: Profile): void {
...
async function updateProfileAction(
  ...
  const taken = new Set(
    profiles.map((p) => p.name).filter((name) => name !== existing.name)
  );
...
async function confirmDelete(
  ...
  const ans = await askLineSimple(
    `${ui.CARD_ACCENT}›${ui.RESET} ${promptText} (y/N): `
  );
...
async function deleteProfileAction(
  ...
  if (
    !(await confirmDelete(ui, `Confirm permanent delete for "${existing.name}"`))
  ) {
    printInfo(ui, "Delete cancelled");
    await pause(ui);
    return;
...
async function handleActivate(
  ...
  out(
    ui,
    `${ui.BOLD}${profile.name}${ui.RESET} — ${toolLabel(profile.tool)}`
  );
  printProfileSummary(ui, profile);
  out(ui, "");
  out(
    ui,
    `${ui.DIM}The tool will start. When you exit, you'll return to the menu.${ui.RESET}`
  );
  await pause(ui);
...
async function handleRunDaemon(ui: CoderUi): Promise<void> {
  ...
  out(
    ui,
    `${ui.DIM}Starting daemon. Press Ctrl+C to stop and return to the menu.${ui.RESET}`
  );
  out(ui, "");
...
async function handleSetWorkDirs(
  ui: CoderUi,
  cfg: Partial<DaemonConfig>
): Promise<void> {
  ...
  for (;;) {
    ...
    if (key === "a" || key === "A") {
      const input = await askLineSimple(
        `${ui.CARD_ACCENT}›${ui.RESET} Add working directory path: `
      );
    ...
    if (key === "d" || key === "D") {
      ...
      const input = await askLineSimple(
        `${ui.CARD_ACCENT}›${ui.RESET} Number to delete: `
      );
      if (input === null) return;
...
async function handleSettings(
  ...
  const maxConcurrency = await promptMaxConcurrency(
    ui,
...
async function askLineSimple(promptText: string): Promise<string | null> {
...
export async function showMainMenu(): Promise<void> {
  ...
  for (;;) {
    ...
    switch (key) {
      // Arrow Up
      case "\x1b[A":
        if (total > 0 && selectedSlot > 0) selectedSlot--;
        break;
      ...
      case "\x1b[B":
        if (total > 0) {
          const lastSlot = Math.min(
            LANDING_PAGE_SIZE - 1,
            total - page * LANDING_PAGE_SIZE - 1
          );
          if (selectedSlot < lastSlot) selectedSlot++;
        }
        break;
